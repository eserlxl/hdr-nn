...
void neuralNetwork::feedForward(const float *pixels) {
    for (size_t j = 0; j < hidden_neurons; j++) {
        ...
        m_hiddenLayerOutputs[j] = logic(Z/((float)inputs+1));
    }
    ...
    for (size_t j = 0; j < output_neurons; j++) {
        ...
        for (size_t i = 0; i < hidden_neurons; i++) {
            Z += m_hiddenLayerOutputs[i] * m_outputLayerWeights[OutputLayerWeightIndex(i, j)];
        }

        m_outputLayerOutputs[j] = logic(Z/((float)hidden_neurons+1));
    }
}